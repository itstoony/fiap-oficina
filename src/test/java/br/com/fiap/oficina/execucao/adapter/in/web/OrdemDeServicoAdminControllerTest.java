...
class OrdemDeServicoAdminControllerTest {
    ...
    @MockitoBean private OrdemDeServicoAdminUseCase useCase;
    ...
    void criar_comDadosValidos_deveRetornarCreated() throws Exception {
        ...
        when(useCase.criar(any())).thenReturn(criarResponse("RECEBIDA"));
    ...
    void listar_deveRetornarListaDeOrdens() throws Exception {
        when(useCase.listar()).thenReturn(List.of(criarResponse("RECEBIDA"), criarResponse("EM_DIAGNOSTICO")));
    ...
    void buscarPorId_osExistente_deveRetornarOk() throws Exception {
        ...
        when(useCase.buscarPorId(id)).thenReturn(criarResponse("RECEBIDA"));
    ...
    void buscarPorId_osInexistente_deveRetornarNotFound() throws Exception {
        UUID id = UUID.randomUUID();
        when(useCase.buscarPorId(id)).thenThrow(new RecursoNaoEncontradoException("OS não encontrada"));
    ...
    void iniciarDiagnostico_transicaoValida_deveRetornarOk() throws Exception {
        UUID id = UUID.randomUUID();
        when(useCase.iniciarDiagnostico(id)).thenReturn(criarResponse("EM_DIAGNOSTICO"));
    ...
    void iniciarDiagnostico_transicaoInvalida_deveRetornarUnprocessableEntity() throws Exception {
        UUID id = UUID.randomUUID();
        when(useCase.iniciarDiagnostico(id)).thenThrow(new RegraDeNegocioException("Transição não permitida"));
    ...
    void iniciarExecucao_comAtendente_deveRetornarOk() throws Exception {
        ...
        when(useCase.iniciarExecucao(eq(id), any())).thenReturn(criarResponse("EM_EXECUCAO"));
    ...
    void cancelar_osExistente_deveRetornarOk() throws Exception {
        UUID id = UUID.randomUUID();
        when(useCase.cancelar(id)).thenReturn(criarResponse("CANCELADA"));
